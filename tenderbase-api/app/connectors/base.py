...
class SourceContext:
    ...
    #: Explicit connector implementation key (``None`` = resolve by type).
...
class ProcurementConnector(ABC):
    ...
    #: ``False`` when the connector's *external* contract has not been verified
    #: against a live service (or is known to be incomplete). Surfaced by
    #: ``GET /sources/connectors`` so operators never mistake an experimental
    #: connector for a supported one.
    production_ready: bool = True
    #: Honest caveat shown next to ``production_ready`` in the API.
